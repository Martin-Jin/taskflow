...
const TASK_CONTENT_FIELDS = [
  ...
  { name: 'notes', type: 'string', description: 'Free-text notes/description, if extra detail was given beyond the title.' },
  {
    ...
    type: 'string',
    description:
      'The single most relevant URL for this task (e.g. an assignment page, submission portal, or doc the task is about), if one was given or found. This mirrors what happens when a user types a bare URL into the title box themselves — it makes the task\'s title a clickable link straight to that URL. Put only the ONE primary URL here; if there is more than one URL, put the rest in notes instead.',
  },
  {
    name: 'estimatedHours',
    type: 'number',
  ...
  },
  {
    ...
    type: 'string',
    description:
      'ISO date (YYYY-MM-DD) before which the task cannot be worked on/scheduled at all — set this when the request or context clearly states the task genuinely is not available/doable until a specific day (e.g. "assignment unlocks on the 10th", "can\'t start until the materials arrive Monday", a course page saying a task opens on a given date), OR when the user directly asks to lock/pin the task to not start before a day (e.g. "lock this to the 10th", "lock to a day", "don\'t start before the 10th", "don\'t schedule it before Monday"). Do not set this just because a task has a due date or a generally preferred day to work on it with no explicit "not before"/"lock" wording — it is for an explicit constraint, not an ordinary scheduling preference.',
  },
...
function buildSystemPrompt(contextMarkdown) {
  return [
    ...
    'This ONLY applies to an actual exam/test/lab SESSION being scheduled — a specific sit-down occurrence that happens once, at a fixed time, on one day (e.g. "Midterm exam on the 12th", "Lab session Tuesday 2pm"). It does NOT apply merely because a task\'s title/project/section mentions "lab" or "test" — a lab report, lab write-up, or any other deliverable that can be worked on across multiple days is a normal due-dated task, not this case. For an actual exam/test/lab session: set dueDate to that day AND enforceDueDate to true (not just dueDate alone). If no specific time was stated, assume a fixedTime/estimatedHours block of 9am–12pm (fixedTime "09:00", estimatedHours 3) as a reasonable default; use the stated time/duration instead when one is given.',
    'Set earliestDate whenever the request clearly means "do not schedule/work on this before day X" — this covers both a hard availability constraint (e.g. an assignment that "unlocks"/"opens"/"is released" on a given date, or something blocked on materials/access arriving) AND direct phrasing like "lock this to the 10th", "lock to a day", "don\'t start before the 10th", or "don\'t let it be worked on before the 10th". Do not set earliestDate merely because a task has a due date or a generally preferred day to start with no explicit "not before"/"lock" framing — it is for an explicit constraint, not an ordinary scheduling preference.',
    'Assignment/brief documents — this applies whenever an attached image/PDF is an assignment brief, syllabus, or project spec, OR the request explicitly asks to turn one into tasks (e.g. "make task(s) for this assignment", "add this as a task", "create tasks from this"): read the document\'s full text/pages, not just a title, before deciding how to break it down. If it describes a single deliverable made of multiple distinct pieces of work (e.g. "proposal", "draft", "implementation", "final report", separately gradable milestones, or a rubric with multiple sections), create one parent create_task for the overall assignment plus one create_task per piece with parentId set to the parent\'s localId, instead of a single flat task — but do not split out a piece that is really just one step of an otherwise single-sitting task, and if the document genuinely describes only one piece of work, one task (no subtasks) is correct. Use dependsOn to chain subtasks that the document implies must happen in order (e.g. a draft/proposal before the final submission) — do not invent an order the document does not state or imply. Copy each piece\'s own stated deadline onto its own dueDate (and enforceDueDate/earliestDate per the rules above) rather than only setting a date on the parent task when the document gives per-piece dates; if only one overall deadline is given, set it on the parent (and on any subtask it clearly applies to). Put any substantial extra detail from the document that does not fit in the title — grading criteria, submission instructions, page/word counts — into that task\'s notes, on whichever of the parent/subtasks it is actually relevant to, not dumped only on the parent. If the document contains URLs (course page, submission portal, reading links, etc.), set the single most relevant one as that task\'s link field — the one a user would actually want to click through to for that specific piece of work — and put any additional URLs in notes.',
    'Whenever the user\'s own request (not just an attached document) mentions or pastes a URL that belongs to a specific task (a link to the assignment, doc, ticket, page, etc. the task is about), set it as that task\'s link field rather than only mentioning it in notes — this is what makes the task title a clickable link, the same as if the user had typed the URL into the title box themselves. If more than one URL is relevant, put the single most important one in link and the rest in notes.',
    '',
    contextMarkdown,
  ].join('\n');
}
