...
export function parseRecurrenceRule(str) {
  ...
  const numericMatch = s.match(new RegExp(`(?:every|ev)!?\\s+(\\d+)\\s*(${unitAlt})\\b`));
  if (numericMatch) {
  ...
  const simpleMatch = s.match(new RegExp(`(?:every|ev)!?\\s+(${unitAlt})\\b`));
  if (simpleMatch) {
  ...
  if ((s.startsWith('every') || s.startsWith('ev')) && hasWeekday) {
...
export function findRecurrencePhrase(text) {
  ...
  const weekdayMatch = s.match(new RegExp(`(?:every|ev)!?\\s+(${DAY_NAMES.join('|')})[a-z]*(?:,\\s*(?:${DAY_NAMES.join('|')})[a-z]*)*`));
  if (weekdayMatch) {
