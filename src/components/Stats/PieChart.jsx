...
export default function PieChart({ data, emptyMessage = 'No data yet.' }) {
  ...
  if (data.length === 0 || total <= 0) {
    return <div style={{ color: 'var(--text-tertiary)', fontSize: 13, padding: '20px 0' }}>{emptyMessage}</div>;
  }
  ...
  return (
    <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
      <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ flexShrink: 0 }}>
        ...
        <text x={CENTER} y={CENTER - 4} textAnchor="middle" fontSize="19" fontWeight="700" fill="var(--text-primary)">
          {total.toFixed(0)}h
        </text>
        <text x={CENTER} y={CENTER + 14} textAnchor="middle" fontSize="10.5" fill="var(--text-tertiary)">
          total
        </text>
      </svg>
