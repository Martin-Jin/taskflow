...
const PALETTE = ['#6c7bf0', '#4fb8e0', '#f0a83e', '#4fbf8b', '#ef5a5a', '#a06cf0', '#e07ba0', '#8bc34a'];
...
function StatCard({ label, value, sublabel, accent }) {
  return (
    <div className="card" style={{ flex: 1, minWidth: 140 }}>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 600, marginTop: 4, color: accent || 'var(--text-primary)' }}>
        {value}
      </div>
...
export default function StatsDashboard() {
  ...
  const projectHoursData = useMemo(() => {
    ...
    for (const b of blocks) {
      ...
      if (!task) continue;
      const name = task.projectId ? projectNameById.get(task.projectId) || task.projectName || 'Unnamed project' : 'No project';
    ...
    <div>
      <div className="stats-cards-row">
        ...
        <StatCard
          ...
          accent={stats.freeCapacityThisWeek < 2 ? 'var(--warning)' : 'var(--success)'}
        />
      </div>

      {stats.overdueRiskTasks.length > 0 && (
        <div className="card" style={{ borderLeft: '3px solid var(--danger)', marginBottom: 16 }}>
          <strong style={{ color: 'var(--danger)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
