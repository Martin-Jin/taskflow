...
export const CHANGELOG = [
  ...
  },
  {
    ...
    changes: [
      'A task with a fixed time can now be scheduled outside your working hours — a 9pm gym session used to silently never get scheduled at all, because the work window filtered it out before the scheduler ever considered it. Setting a fixed time is a more specific instruction than the general "these are my hours" rule, so it wins.',
      'It still can’t override anything genuinely occupied: sleep and every other routine, a real calendar event, and anything already on your calendar are all still respected, so a late fixed time can’t land inside sleep or double-book a meeting.',
      'Routine blocks in the calendar are full width again, with their name and time hidden until you point at them — the earlier half-width version fixed the same clutter but squeezed the label and nudged neighbouring blocks; hiding the label until hover keeps the block’s full size while cutting the same repetition.',
      'Fixed a leftover styling bug in Settings → Fixed routines where an active routine’s name rendered inside a boxed button instead of as plain text.',
    ],
  },
  {
    version: '9.4.1',
    date: '2026-08-23',
    title: 'Stats no longer warns about a buffer that doesn’t exist',
    changes: [
      'A task marked "Must be done on due date" is no longer listed as at risk of missing its buffer deadline. That setting collapses the whole planning window onto the due date, so there is no buffer to miss — Stats was warning about one anyway, while the scheduler treated the same task as perfectly on time.',
      'Stats now reads the deadline from the scheduler itself instead of recalculating it, so the warning and the plan can’t disagree again. As a side effect an undated sub-task now inherits its parent’s deadline here too, which the old calculation couldn’t see at all.',
    ],
  },
  {
    version: '9.4.0',
    date: '2026-08-23',
    title: 'Calendar: short events cluster, and small blocks keep their titles',
    changes: [
      'Fixed short calendar events rendering as unreadable slivers instead of grouping together. Only tasks were ever eligible to cluster, so three 10-minute events sat at 10-17px each with no title you could read, while three identical task blocks correctly grouped into one chip. Whether something can stand on its own is a question of how many pixels it gets, not which kind of thing it is.',
      'One-hour events still never group — that over-grouping was fixed previously and there are now tests pinning both directions, so neither can come back silently.',
      'A block that is slightly too short for its title now shows it at a marginally smaller size rather than disappearing into an anonymous "N tasks" chip. Below that it still groups, because no font size makes 12 pixels readable.',
      'A chip holding calendar events now says "3 items" rather than "3 tasks".',
      'Fixed routines in Settings: the pause control moved off the coloured dot beside the name and sits with Delete at the block’s edge, a paused routine stays visibly paused, and the day badge reads "All days" instead of "Not this day".',
    ],
  },
  {
    version: '9.3.0',
    date: '2026-08-23',
    title: 'Routines per weekday, and quieter routine blocks',
    changes: [
      'Fixed routines can now be set for individual weekdays. A day picker beside the timeline scopes what you see, and anything you draw while a day is selected applies to that day alone — so "gym on Tuesdays" is one drag. Routines always had per-day support internally; there was just no way to reach it, so every new routine took all seven days.',
      'Each day shows how many routines it carries, and in "All days" view a routine that isn’t daily is badged with its days — previously a Tuesday-only routine looked identical to a daily one, so dragging it seemed to change every day.',
      'Routine blocks in the calendar now take half the column instead of all of it. At full width a routine read as the main event, and on a day with nothing else scheduled it looked exactly like a booked block; half leaves the rest of the column visibly free, which is what "available" should look like. They also sit further off the column edge, and a long routine name scrolls to reveal itself when you point at it.',
    ],
  },
  {
    version: '9.2.0',
    date: '2026-08-23',
    title: 'Pick your first day of the week, and a fixed weekly review',
    changes: [
      'New setting: first day of the week, Sunday or Monday (Settings → Scheduling rules). It drives the Calendar’s week view, the weekly review, and every “this week” figure on the Dashboard and Stats — previously all of those were hardcoded to Sunday.',
      'Fixed the weekly review calling tasks “slipped” when they were due today and the day had barely started. A task due today hasn’t slipped; it now sits in no bucket until the day is actually over, matching what the rest of the app means by overdue.',
      'The weekly review also now reviews the week that just ended when you open it on the first day of a new week, instead of showing you a week that’s a few hours old and empty.',
      'Once you’ve closed a review, a quiet “View this week’s review” row under the progress rings keeps it one click away — previously the prompt disappearing was the only way in, short of the command palette.',
      'Lighter row actions in the review (one bordered group instead of three separate buttons) and more breathing room under its date line.',
    ],
  },
  {
    version: '9.1.0',
