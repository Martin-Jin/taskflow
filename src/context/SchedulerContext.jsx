...
import { resolveEventId, truncateRuleUntil } from '../utils/recurrenceExpansion';
import { dedupeEventsByOccurrence } from '../utils/eventUtils';
import { applyEventScopeUpdate, splitSeriesAtOccurrence } from '../utils/eventScopeUpdate';
...
export function SchedulerProvider({ children }) {
  ...
  //
  // Same useLocalEditTrackedState treatment as sections/projects/labels below
  // (see that hook's doc comment) now that events live-sync across devices
  // through the same race-guarded path tasks/blocks and those settings do —
  // every LOCAL mutator (addManualEvent/updateEvent/deleteEvent/
  // setEventIgnored) needs to bump localNonUndoEditIdRef so useCloudSync's
  // race guard can tell a local event edit apart from an incoming remote one,
  // exactly the gap useLocalEditTrackedState was built to close structurally
  // for every OTHER cloud-synced field. `setEventsRaw` (untracked) is what
  // useCloudSync itself uses to apply a merged/restored `events` value,
  // mirroring every other setX/setXRaw pair passed into that hook.
  const [events, setEvents, setEventsRaw] = useLocalEditTrackedState(
    useState(() => dedupeEventsByOccurrence(loadPersisted('events', null) ?? getMockEvents())),
    localNonUndoEditIdRef
  );
  ...
  // Same tombstone-hiding treatment as visibleTasks above, now that events
  // carry their own deletion tombstones too (see utils/eventTombstones.js).
  // Every consumer of useScheduler() gets this filtered view via `value.events`
  // below, never the raw array — a tombstoned event simply never appears to
  // the calendar grid, search, or anywhere else, the same way a tombstoned
  // task is already invisible to its own consumers.
  //
  // CRITICAL: same rule as visibleTasks — this filtering must NEVER reach
  // `cloudSyncState.events`/`stateRef`-equivalent data, the raw `events`
  // binding used internally by addManualEvent/updateEvent/deleteEvent/
  // setEventIgnored, or what's persisted (savePersisted('events', ...)). The
  ...
  // ---- Deleted event (tombstone) retention sweep ---------------------------
  // Same shape and reasoning as the deleted-task sweep just above, for
  // CalendarEvents now that they carry their own deletion tombstones (see
  // deleteEvent/utils/eventTombstones.js). A tombstoned event stays in
  // `events` — never physically removed at delete time — so the per-event
  // cross-device merge (utils/eventMerge.js) has time to see the deletion
  // before it's gone for good. Once that window (RETENTION_DAYS_DELETED_EVENTS)
  // has passed, this actually drops it from the array. Runs once on mount,
  // reading `events` directly rather than through `visibleEvents` — the whole
  // point of this sweep is to find tombstones, which that filtered view never
  // contains. Uses the tracked `setEvents` (not a raw/untracked setter): this
  // is an ordinary local mutation of this device's own data, the same as the
  // migration effects above, and should participate in the cloud-sync race
  // guard like any other local edit.
  useEffect(() => {
    setEvents((prev) => {
      const staleTombstoneIds = new Set(
        prev.filter((e) => isStaleEventTombstone(e, RETENTION_DAYS_DELETED_EVENTS)).map((e) => e.id)
      );
      if (staleTombstoneIds.size === 0) return prev;
      return prev.filter((e) => !staleTombstoneIds.has(e.id));
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // except `theme` and `accentSeed`, which stay separate params instead (see
  // useCloudSync's own JSDoc) since live sync leaves those two to
  // ThemeContext's own independent sync. `events` joined this bundle once the
  // per-event timestamp+tombstone merge (utils/eventMerge.js/
  ...
  const cloudSyncState = useMemo(
    () => ({
      ...
      // already has concurrent writers.
      ...
      trash,
      sharedProjectIds,
      // CalendarEvents have no shared-project equivalent (they're always
  ...
  const {
    ...
    setAccentSeed,
    events,
    // RAW/untracked, matching every other setXRaw passed into this hook —
    // useCloudSync only ever calls this to APPLY remote/backup data
    // (applyBackupPayload's restore, and the events-fallback-from-backup
  ...
  const deleteEvent = useCallback(
    (eventId, scope = 'all') => {
      ...
      if (!isVirtual || scope === 'all') {
        ...
        // Tombstone rather than remove — see utils/eventTombstones.js. This
        // is the branch that actually deletes a real row (a manual event, a
        // synthetic series' fan-out, or a true-RRULE series' 'all' scope,
        // which deletes the master row); the 'this'/'following' branches
        // further below never remove a row at all — they only ever edit the
        // master's own overrides/recurrenceRule fields, which already sync
        // correctly as an ordinary field-level edit via mergeEventsByUpdatedAt,
        // so tombstoning doesn't apply there. Keeping the row (instead of
        // filtering it out) lets the cross-device merge tell "deleted here"
        // apart from "never existed on this device" the same way a task
        // deletion already does.
        setEvents((prev) => tombstoneEvents(prev, deleteIds, new Date().toISOString()));
        deleteTargets.forEach((t) => {
          if (googleConnected && t.googleEventId) {
            ...
            deleteCalendarEvent(t.googleEventId, t.calendarId).catch((err) => {
              ...
              // Also un-tombstone THIS specific target locally now, rather
              // than leaving it deleted until the next poll silently
              // resurrects it with no explanation — only this target
              // reverts, so siblings in a multi-event fan-out that deleted
              // successfully stay tombstoned. Replaces the tombstone row
              // with the original pre-delete object `t` (rather than the
              // old "add back if missing" check, which no longer applies now
              // that the row was never removed, just tombstoned).
              setEvents((prev) => prev.map((e) => (e.id === t.id ? t : e)));
              setNotification({
                type: 'error',
      ...
      if (scope === 'this') {
        // One timestamp for this whole delete call, reused below if the
        // Google push fails and the override has to be reverted — same
        // "compute once, reuse" convention updateEvent's `stamped` follows.
        // Stamped both INSIDE the per-occurrence override (this is what the
        // occurrence itself actually changed) AND on the master row's own
        // top-level `localUpdatedAt` — the second stamp is what
        // mergeEventsByUpdatedAt (utils/eventMerge.js) actually reads to
        // decide which device's copy of this WHOLE ROW wins a cross-device
        // merge; it has no idea an `overrides` entry nested inside is what
        // changed, so without it this edit could lose to (or be silently
        // outranked by) an older edit on another device that happens to
        // carry a newer top-level stamp. Matches applyEventScopeUpdate's
        // 'this' branch (used by updateEvent), which does the same for the
        // same reason. Harmless for the Google push just below: it only ever
        // reads the per-occurrence override, never the master's other
        // top-level fields, for a single-occurrence delete.
        ...
        setEvents((prev) =>
          prev.map((e) =>
            e.id === masterId
              ? {
                  ...e,
                  localUpdatedAt: deleteStamp,
                  overrides: {
                    ...(e.overrides || {}),
                    [occurrenceDate]: { ...(e.overrides?.[occurrenceDate]), deleted: true, localUpdatedAt: deleteStamp },
                  },
                }
              : e
          )
        ...
        if (googleConnected && master.googleEventId) {
          ...
          deleteCalendarEventInstance(master, occurrenceDate).catch((err) => {
            ...
            // silently brings it back with no explanation. This revert is
            // itself a genuine new local change (the event's real current
            // state just changed again, back to "not deleted") that other
            // devices need to see, so it gets its OWN fresh stamp — on both
            // the override AND the master's own top-level `localUpdatedAt`
            // (same reasoning as the delete above: that's the field the
            // cross-device merge actually compares) — rather than reusing
            // `deleteStamp` or the prior override's older timestamp; a stale
            // remote copy must not be able to out-rank a revert that happens
            // after it. When there was no prior override at all (the
            // occurrence had no edits before this delete), the override entry
            // is removed entirely rather than left as a bare timestamp, since
            // an override with no real fields besides `localUpdatedAt` would
            // be indistinguishable from "no override" everywhere else that
            // reads it (see recurrenceExpansion.js).
            setEvents((prev) =>
              prev.map((e) => {
                if (e.id !== masterId) return e;
                const overrides = { ...(e.overrides || {}) };
                const priorOverride = master.overrides?.[occurrenceDate];
                if (priorOverride) overrides[occurrenceDate] = { ...priorOverride, localUpdatedAt: new Date().toISOString() };
                else delete overrides[occurrenceDate];
                return { ...e, localUpdatedAt: new Date().toISOString(), overrides };
              })
            );
            setNotification({
              type: 'error',
      ...
      const truncatedMaster = {
        ...master,
        recurrenceRule: truncateRuleUntil(master.recurrenceRule, dayBefore),
      ...
      if (googleConnected) {
        pushEventToCalendar(truncatedMaster)
          ...
          .catch((err) => {
            ...
            // restores the "deleted" occurrences with no explanation. Like the
            // 'this'-scope revert above, this is itself a fresh genuine local
            // change (the series' real current state reverted back to
            // un-truncated), so it gets its own new stamp rather than
            // reusing `master`'s old one — otherwise a stale remote copy
            // dated between the original edit and this revert could
            // incorrectly win a later merge against the reverted truth.
            setEvents((prev) => prev.map((e) => (e.id === masterId ? { ...master, localUpdatedAt: new Date().toISOString() } : e)));
            setNotification({
              type: 'error',
              message: `Couldn't delete "${master.title}" from Google Calendar: ${err?.message || err}`,
            });
  ...
  const setEventIgnored = useCallback((event, ignored, scope = 'this') => {
    ...
    // One timestamp for this whole call, reused across every branch/event it
    // touches — same "compute once, reuse" convention updateEvent's `stamped`
    // follows just above.
    const ignoreStamp = new Date().toISOString();

    if (!isVirtual) {
      if (scope === 'this' || !event.seriesId) {
        setEvents((prev) => prev.map((e) => (e.id === event.id ? { ...e, isFreeTime: ignored, localUpdatedAt: ignoreStamp } : e)));
        return;
      }
      setEvents((prev) =>
        prev.map((e) => {
          ...
          return { ...e, isFreeTime: ignored, localUpdatedAt: ignoreStamp };
        })
      );
      return;
    }
    ...
    if (scope === 'all') {
      setEvents((prev) => prev.map((e) => (e.id === masterId ? { ...e, isFreeTime: ignored, localUpdatedAt: ignoreStamp } : e)));
      return;
    }
    ...
    if (scope === 'following') {
      ...
      setEvents((prev) => {
        ...
        // Matches updateEvent's own convention for the same helper: the
        // stamp rides inside `fieldUpdates` alongside the real field being
        // changed, since splitSeriesAtOccurrence spreads `fieldUpdates` onto
        // the new master and doesn't stamp anything itself.
        return splitSeriesAtOccurrence(prev, master, occurrenceDate, { isFreeTime: ignored, localUpdatedAt: ignoreStamp }, splitId).events;
      });
      return;
    }

    // scope === 'this' — stamped both INSIDE the per-occurrence override AND
    // on the master row's own top-level `localUpdatedAt`, matching
    // applyEventScopeUpdate's 'this' branch (see deleteEvent's 'this' branch
    // above for the same convention and the reasoning behind it: the merge
    // only ever compares the master's top-level stamp, so it has to move too
    // even though every other top-level field on this row is untouched).
    setEvents((prev) =>
      prev.map((e) =>
        e.id === masterId
          ? {
              ...e,
              localUpdatedAt: ignoreStamp,
              overrides: {
                ...(e.overrides || {}),
                [occurrenceDate]: { ...(e.overrides?.[occurrenceDate]), isFreeTime: ignored, localUpdatedAt: ignoreStamp },
              },
            }
          : e
      )
